...
package Kernel::System::DynamicField::Driver::HTMLText;
...
sub new {
    ...
    my $DynamicFieldDriverExtensions
        = $Kernel::OM->Get('Kernel::Config')->Get('DynamicFields::Extension::Driver::HTMLText');
...
sub EditFieldRender {
    ...
    # process tags as possible
    $Value =~ s/\<OTRS_DATA_(\w+)\>/$Param{LayoutObject}->{$1}/ixg;
    ...
    my $FieldClass = 'DynamicFieldHTMLText';
    ...
    my $HTMLString = <<"EOF";
<textarea class="$FieldClass" id="$FieldName" name="$FieldName" rows="$RowsNumber" cols="$ColsNumber" data-maxlength="$MaxLength" readonly="readonly">$ValueEscaped</textarea>
<div class="$FieldClass">$Value</div>
EOF
